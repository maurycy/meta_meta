module MetaMeta
  module ClassMethods
    ...
    def chain(reload=false, &block)
      ...
      # Evaluate the block.
      v.instance_eval(&block) if block_given?
    ...
    def with_chain(obj, ego, &block)
      ...
      ego.class.chain = obj
      ret = ego.instance_eval(&block)
      ego.class.chain = v
      ret
    end

    def without_chain(ego, name, &block)
      if name.nil?
        with_chain(nil, ego, &block)
      else
